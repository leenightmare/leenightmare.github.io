...
1）@Configuration ： 放在一个类的上面，表示这个类是作为配置文件使用的。(把这个类当做是xml文件)

2）@Bean：声明对象，把对象注入到容器中。(作用类似bean标签)
...
public class SpringConfig {
     ...
     *
     *	相当于xml中的配置
     *	<bean id="createStudent" class="com.bjpowernode.vo.Student">	
...
没有 xml 配置文件，获取容器的方式，使用 java 类代替 xml 配置文件的作用

传统spring的xml配置方式获取容器

```
ApplicationContext ctx = new ClassPathXmlApplicationContext("application.xml");
```

springboot注解方式获取容器(不常用)
...
@ImportResource 作用导入其他的xml配置文件， 等同于 xml 文件的 resources
...
@ComponentScan,相当于定义注解扫描器 <context:compont-scan:basePackage="xxx"/ >
...
也就是这个@SpringBootApplication注解的类,是一个配置类,同时声明了注解扫描



用xml的角度来看

@SpringBootApplication开局自带application.xml,并定义了<context:compont-scan:basePackage="路径是相对该注解所在位置的包和子包"/ >
...
在springboot中，可以引用配置文件的内容。 key 来自 application.properties(yml)



⭕疑惑？？？？为什么不用加@PropertyResource
...
3) 创建一个存放jsp的目录（与java、resources同级），一般叫做webapp，在idea中注意看文件夹图标有没有web标识。
...
<resources>
	<resource>
		...
		<includes>
			意思是把src/main/webapp下的所有文件,编译后放到META-INF/resources下
			<include>**/*.*<include/>
		<includes/>
	</resource>
</resources>
...
```
在controller中,返回视图..对于这里的路径问题,还是没搞懂????????????
return "index";
```
...
application.properties文件配置mybais信息
...
4) 在pom.xml中指定 把resources目录中的文件 ， 编译到目标目录中  (这一步好像不用也可以?)
...
```
http://localhost:8008/student/1001/classone
```
...
id,@PathVariable String classname){
    //...
    //@PathVariable会获取studentId的值赋值给id
    //获取classname的值赋值给classname
    //也就是说当{xxx}里的名字和形参的名字相同时,@PathVariable可以省略value=""
}

//添加
@PostMapping(value = "/student/{studentId}/{classname}")
//更新
@PutMapping(value = "/student/{studentId}/{classname}")
//删除
...
对于ajax来说,直接指定type的请求类型就可以,甚至不用指定spring.mvc.hiddenmethod.filter.enabled=true 
...
需要添加一个隐藏域name="_method" value="put"

并且开启spring.mvc.hiddenmethod.filter.enabled=true 

```html
<form action="user/zhangsan" method="post">
    <input type="hidden" name="_method" value="put">
...
在springboot项目创建时,在NoSQL中勾选第一个,即Spring Data Redis(Access+Driver)
...
## 6.2 在Controller使用redis

1. 在controller中定义成员变量private RedisTemplate redisTemplate;并且加上注解自动注入
2. 在方法中使用redisTemplate.opsForxxx返回的操作类型(string,set,hash....)对象
3. 调用对象的方法操作redis
...
## 6.3 对比 StringRedisTemplate 和 RedisTemplate 
...
创建SpringBoot项目      ⭕注意如果没有勾选web依赖时,是没有jackson相关依赖的
...
<dependencies>
   ...
   <dependency>
      ...
      <type>pom</type>
      <exclusions>
         <!-- 排除log4j依赖,因为其他依赖也引用了log4j,为了解决冲突,这里必须排除 -->
         <exclusion>
 ...
 * 使用dubbo中的注解暴露服务
 * @Component 可以不用加
 * interfaceClass = StudentService.class可以省略
 */
...
#如果使用的是远程的zookeeper,测试时最好设置timeout
...
⭕这种war包的形式部署到tomcat中,使用的端口和contextPath是tomcat配置中的

​	 即application.properties中的server.port在这里没有作用



⭕为避免bug   tomcat的版本要和springboot内置的tomcat版本(在maven的依赖管理中可以看到)一样
...
⭕这种方式运行的项目,使用的是springboot内置的tomcat
